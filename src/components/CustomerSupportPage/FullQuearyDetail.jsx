...
import { DownloadAttachment } from "../../lib/store";
...
const originApi =
  process.env.REACT_APP_OA_URL || "https://temp.beautyfashionsales.com";
// const originApi = "http://localhost:2611";
function FullQuearyDetail({ data, setRest, attachmentUrls }) {
  ...
  const downloadFile = async (attachmentId, attachmentName) => {
    try {
      ...
      link.setAttribute(
        "download",
        attachmentName || `attachment_${attachmentId}`
      );
          ...
          <div className="col-lg-9 col-md-9 col-sm-12">
            <div className={Detail.LeftMainDiv}>
              ...
              <p style={{ marginTop: "1rem" }}>
                {data.Description?.split("User Desc:")?.length == 2 ? (
                  <>
                    {
                      data.Description.split("User Desc:")[1].split(
                        "Issue Desc:"
                      )[1]
                    }
                    <br />
