      ...
      .then((user) => {
        if (data.orderStatusForm.opportunityId) {
          let rawData = {
            ...
            opportunity_id: data.orderStatusForm.opportunityId,
          };
          getOrderDetailId({ rawData })
            .then((res) => {
              setOrderDetail(res);
            })
            .catch((orderErr) => {
  ...
  if (supportTicketData?.orderStatusForm?.opportunityId) {
    ...
    return (
      <section>
            ...
            <div className="d-flex flex-column gap-3">
              ...
              <div className="d-flex justify-content-around ">
                <button
                  ...
                  onClick={() => onSubmitHandler(confirm)}
                >
                  Submit
                </button>
                <button
        ...
        >
          <Form>
            <h2 className={stylesV2.TitleHolder}>
              <b>Order Po</b> {orderDetails.PO_Number__c}{" "}
              ...
              <span style={{ float: "right", fontWeight: "bold" }}>
                ...
                <p style={{ margin: 0, fontWeight: "initial" }}>
                  {supportTicketData?.orderStatusForm?.reason}
                </p>
              </span>
            </h2>
            <p className={stylesV2.titleSubHolder}>
              {DateConvert(orderDetails.CreatedDate, true)}
            </p>
            <div className={`${stylesV2.dFlex} ${stylesV2.spaceBetween}`}>
              <div className={stylesV2.itemsHolder}>
                <div className={stylesV2.detailsCardTitleHolder}>
                  ...
                  <div className={stylesV2.itemHolder}>
                    {orderDetails.OpportunityLineItems.map((item) => (
                      <div
                        className={`${stylesV2.dFlex} ${stylesV2.borderBottom}`}
                      >
                        <div className={stylesV2.imgHolder}>
                          <img
                            src={
                              item.ContentDownloadUrl ??
                              "https://b2b.beautyfashionsales.com/dummy.png"
                            }
                            alt="dummy"
                          />
                        </div>
                        <div className={stylesV2.productDetailsHolder}>
                          <p className={stylesV2.productTitleHolder}>
                            {item.Name.split(orderDetails.Name)}&nbsp;
                            <span className={stylesV2.contentHolder}>
                              ({item.ProductCode})
                            </span>
                          </p>
                          <div
                            className={`${stylesV2.dFlex} ${stylesV2.spaceEnd} mt-2`}
                          >
                            <p
                              className={`${stylesV2.productTitleHolder} ml-4`}
                            >
                              <small style={{ fontSize: "9px" }}>qty</small>{" "}
                              {item.Quantity} x {formentAcmount(item.UnitPrice)}
                            </p>
                            <p
                              className={`${stylesV2.productTitleHolder} ml-4`}
                            >
                              <small style={{ fontSize: "9px" }}>pirce</small>{" "}
                              {formentAcmount(item.TotalPrice)}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className={`${stylesV2.detailsCardTitleHolder}`}>
                  <b>Order Summary</b>
                  <p
                    className={`${stylesV2.dFlex} ${stylesV2.spaceBetween} mt-2 mb-1`}
                  >
                    <span>Total</span>{" "}
                    <span>{formentAcmount(orderDetails.Amount)}</span>
                  </p>
                </div>
              </div>
              <div className={stylesV2.orderDetailsHolder}>
                ...
                <div className={stylesV2.detailsCardTitleHolder}>
                  <b>Brand</b>
                  <p className="mb-2 mt-1">
                    {orderDetails.ManufacturerName__c}
                  </p>
                </div>
                <div className={stylesV2.detailsCardTitleHolder}>
                  <b>Shipping Address</b>
                  <p className="mb-2 mt-1">
                    {orderDetails.Shipping_Street__c},<br />
                    {orderDetails.Shipping_City__c},
                    {orderDetails.Shipping_State__c},{" "}
                    {orderDetails.Shipping_Country__c}
                    <br /> {orderDetails.Shipping_Zip__c}
                  </p>
                </div>
                {/* <div className={stylesV2.detailsCardTitleHolder}>
              <b>Description</b>
              ...
              <Link
                to={"/orderStatus"}
                className={`${styles.btn} ${stylesV2.backButton}`}
              >
                Cancel
              </Link>
              <input
                type="submit"
                className={`${styles.btn} ${stylesV2.submitButton}`}
                value={"Submit"}
                disabled={activeBtn}
              />
            </div>
              ...
              {(formProps) => (
                <div className={styles.container}>
                  <Form className={styles.formContainer}>
                    <b className={styles.containerTitle}>
                      {supportTicketData?.orderStatusForm?.reason ==
                      "Status of Order"
                        ? "Status of the order"
                        : supportTicketData?.orderStatusForm?.reason ==
